...
int initserver(int port)
{
  ...
  setsockopt(sock,SOL_SOCKET,SO_REUSEADDR,&opt,len);//设置监听端口复用，以免错误退出后端口不能使用

  struct sockaddr_in servaddr;
  servaddr.sin_family = AF_INET;
  servaddr.sin_addr.s_addr = htonl(INADDR_ANY);
  servaddr.sin_port = htons(port);//监听端口   信息结构体初始化

  if (bind(sock,(struct sockaddr *)&servaddr,sizeof(servaddr)) < 0 )//绑定文件描述符与上述结构体信息
  {
    perror("bind() failed"); close(sock); return -1;
  }

  if (listen(sock,5) != 0 )//开始监听  第二个参数是listen监听的队列大小
  {
    perror("listen() failed"); close(sock); return -1;
  }

  return sock;//返回创建的监听描述符
}
